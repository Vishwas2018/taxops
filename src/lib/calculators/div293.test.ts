...
describe("calculateDiv293", () => {
  it("returns zero liability when both inputs are zero", () => {
    const result = calculateDiv293(
      { div293Income: 0, concessionalContributions: 0 },
      config,
    );
  ...
  it("throws on negative inputs", () => {
    expect(() =>
      calculateDiv293({ div293Income: -1, concessionalContributions: 0 }, config),
    ).toThrow();
    expect(() =>
      calculateDiv293({ div293Income: 0, concessionalContributions: -1 }, config),
    ).toThrow();
  });
  ...
  describe("straddling the $250,000 threshold", () => {
    it("combined income exactly at the threshold owes nothing", () => {
      const result = calculateDiv293(
        { div293Income: 225_000, concessionalContributions: 25_000 },
        config,
      );
    ...
    it("one dollar over the threshold triggers a small liability", () => {
      const result = calculateDiv293(
        { div293Income: 225_001, concessionalContributions: 25_000 },
        config,
      );
  ...
  it("caps low-tax contributions at the concessional contributions cap", () => {
    ...
    const result = calculateDiv293(
      { div293Income: 280_000, concessionalContributions: 35_000 },
      config,
    );
  ...
  it("golden file: $240,000 div293Income, $25,000 concessional contributions", () => {
    ...
    expect(result.div293Income).toBe(240_000);
